...
only a new `delete_expense_by_id` mutation helper is added to `database/queries.py`.
...
## Files to change
- `database/queries.py`
  - Add `delete_expense_by_id(expense_id, user_id)` — issues a parameterised
    `DELETE FROM expenses WHERE id = ? AND user_id = ?` to ensure ownership;
    commits and closes the connection.

- `app.py`
  - Import `delete_expense_by_id` from `database.queries`.
  - Replace the GET-only placeholder at `/expenses/<int:id>/delete` with a
    ...
    - Call `delete_expense_by_id(id, session["user_id"])`.
...
- `static/css/profile.css`
  - Add a `.btn-delete` style using CSS variables for danger colour. Never hardcode hex values.
...
- `delete_expense_by_id` must scope its `DELETE` to `id = ? AND user_id = ?` as an
...
|---|---|---|
| `delete_expense_by_id` | valid `expense_id`, correct `user_id` | row removed from DB |
| `delete_expense_by_id` | valid `expense_id`, wrong `user_id` | row remains in DB (0 rows deleted, no error raised) |
| `delete_expense_by_id` | non-existent `expense_id` | no error raised, DB unchanged |
